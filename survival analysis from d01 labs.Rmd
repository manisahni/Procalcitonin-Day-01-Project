...
```{r CHF AKI RIFLE 1 GLM, cache=TRUE}
...
```{r CHF AKI RIFLE 2 GLM ODDS RATIO TABLE (d2maxcr/Cr.d01 >2), cache=TRUE, warning=FALSE}
...
```{r CHF RIFLE 3 AKI GLM, cache=TRUE,warning=TRUE}
...
```{r CHF AKI RIFLE 3 all subsets, cache=TRUE,warning=FALSE}
library(MASS)
library(leaps)
attach(chfnoesrd)
...
```{r Global  RIFLE 3 Renal Failure,cache=TRUE,warning=FALSE}
...
```{r Global  RIFLE 2 Renal Failure, cache=TRUE,warning=FALSE}
